...
impl Ell1D {
     ...
     * @param[in] u
     */
    auto new(double l, double u) -> Self {
        const auto r = (u - l) / 2.0;
        const auto xc = l + r;
        Ell1D { r, xc }
    }
    ...
    auto update_single(const double& grad, double) -> (const CutStatus& b0, double) {
        ...
        const auto tau = if (g < 0.0) { -temp } else { temp };
        const auto tsq = tau * tau;

        if (beta == 0.0) {
            this->r /= 2.0;
            this->xc += if (g > 0.0) { -this->r } else { this->r };
        ...
        const auto u = if (g > 0.0) { bound } else { this->xc + this->r };
        const auto l = if (g > 0.0) { this->xc - this->r } else { bound };
...
// impl UpdateByCutChoices<Ell1D> for (double, Option<double>) {
//     using ArrayType = Arr;
//     auto update_by(mut Ell1const D& ell, Self::ArrayType) -> (const CutStatus& grad, double) {
//         const auto beta = self;
