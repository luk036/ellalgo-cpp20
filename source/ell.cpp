 ...
 * @param[in] x
 */
Ell::Ell(Arr1 val, Arr1 xc) :
    Ell{1.0, xt::diag(val), std::move(xc)}
    {}
 ...
 * @return (i32, double)
 */
auto Ell::update_single(const Arr1& grad, const double& beta) -> std::pair<CutStatus, double> {
 ...
 * @return (i32, double)
 */
auto Ell::update_parallel(const Arr1& grad, const std::pair<double, std::optional<double>>& beta) -> std::pair<CutStatus, double> {
    // const auto [grad, beta] = cut;
    auto mq_g = Arr1{xt::zeros<double>({this->n})}; // initial x0
    auto omega = 0.0;
    for (auto i : py::range(this->n)) {
    ...
    const auto status = b1_opt ?
        this->helper.calc_ll_core(b0, *b1_opt)
    :
